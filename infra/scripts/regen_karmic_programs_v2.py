...
_REFERENCE_FULL = {
    ...
    "description": (
        "В прошлом воплощении задача души могла не быть завершена — "
        "возможен сценарий с прерванным воплощением (в утробе или раннем "
        "детстве, до 7–10 лет) либо со сценарием душевного выбора, при "
        "котором собственные ресурсы (включая материальные) расходовались "
        "поверхностно, без углубления в зрелые задачи. В обоих случаях в "
        "эту жизнь приходит особенная привязанность к материнской фигуре, "
        "тема поиска материнского тепла в партнёре, и тонкое внутреннее "
        "ощущение «не успеть» — оно может ощущаться как давление времени."
    ),
    "manifestations": (
        "В этой жизни программа часто проявляется как сильная связь с "
        "материнской фигурой — иногда близкая к созависимости, иногда как "
        "тяга найти материнское тепло в партнёре. Внутренний ребёнок здесь "
        "обычно требует дополнительного внимания: возможна склонность к "
        "избеганию взрослой ответственности, желание «побыть ребёнком "
        "подольше». Тема материнства может звучать через значимые решения "
        "о детях — собственные или у близких. Часто присутствует ощущение "
        "«не успеваю», и эгоистичный полюс «жить только для себя» может "
        "включаться как защита от внутреннего ребёнка, который чего-то "
        "недополучил."
    ),
    "how_to_heal": (
...
описании. Сохрани её суть, тематику, ТОЧНОСТЬ диагностики. Просто пиши
...
СМЯГЧЁННЫЙ ТОН (КРИТИЧНО — платящий читает это про себя):
- НЕ пиши «ты подчиняла», «ты разрушал», «ты манипулировал» —
  это звучит как обвинение, бьёт по доверию к продукту.
  Пиши: «эта энергия в прошлом воплощении проявлялась как подчинение»,
  «душа несла программу разрушения», «программа работала через
  манипуляции».
- НЕ давай категоричные диагнозы «зависимости, депрессии, саморазрушение».
  Пиши: «склонность к зависимостям», «риск депрессивных состояний»,
  «возможные проявления — …», «тенденция к…».
- Дистанцируй прошлое воплощение от текущего «ты». «Душа в прошлом
  была/проживала/проявляла», а не «ты был/ты делал».
- Цель смягчения — чтобы читатель увидел не обвинение, а карту
  энергии и возможность работы с программой. Тон — психотерапевт на
  первой консультации с уязвимым клиентом: точно, поддерживающе,
  без морализаторства.
