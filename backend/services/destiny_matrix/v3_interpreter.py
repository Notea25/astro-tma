...
def _arcana_canon_block() -> str:
    ...
    ── R-1 ─ Канон именования арканов (продуктовое правило, locked) ─
    Переименованы ТОЛЬКО тяжёлые/негативные арканы. Это не баг
    асимметрии, это принцип:

      12  Повешенный  →  Новое видение
      13  Смерть      →  Трансформация
      15  Дьявол      →  Проявление
      16  Башня       →  Духовное преображение
      18  Луна        →  Магия
      20  Суд         →  Ясно знание
      22  Шут/Дурак   →  Уровневая свобода

    Позитивные арканы — 17 Звезда и 19 Солнце — остаются под своими
    именами НАМЕРЕННО (Ладини их НЕ переименовала). Не переименовывать:
    это удвоит контент и сломает позитивный тон ради ложной симметрии.

    ── R-2 ─ Логика лента/полировщика (locked) ─
    Полировщик (services.destiny_matrix.text_fix) флагует/заменяет
    имена из семёрки R-1 ТОЛЬКО когда они стоят в роли имени аркана
    (через number-anchored pattern «N (X)» или whole-word в прозе).
    «Луна / солнце / звезда» в прямом смысле (фазы, циклы, свет)
    проходят без правок. «Звезда»/«Солнце» как имена арканов 17/19
    тоже легальны.
    """
...
Когда называешь АРКАН — только каноническое имя из таблицы выше:
«аркан 18 (Магия)», НЕ «Луна»; «аркан 5 (Учитель)», НЕ «Иерофант»;
«аркан 12 (Новое видение)», НЕ «Повешенный»; «аркан 22 (Уровневая
свобода)», НЕ «Шут / Дурак». Запрещённые классические имена арканов
(их использовать как ИМЯ АРКАНА нельзя): Иерофант, Жрец, Дьявол,
Повешенный, Шут, Дурак, Отшельник, Колесница, Башня, Смерть,
Умеренность, Колесо Фортуны, Верховная Жрица, Суд.

ВАЖНО (исключение): слова «Луна», «Солнце», «Звезда» свободно
используются В ПРЯМОМ СМЫСЛЕ — фазы Луны, лунные циклы, солнечный
свет, под звёздами. Запрет касается ТОЛЬКО их употребления как
RW-альтернативы каноническому имени: писать «фазы Магии» вместо
«фазы Луны» — ошибка (аркан 18 по Ладини = Магия, не Луна).

ПРИМЕЧАНИЕ ПО 17 И 19: «Звезда» и «Солнце» — это И ЕСТЬ канонические
имена арканов 17 и 19 у Ладини (она НЕ переименовала их, в отличие
от 18→Магия). Канон сходится с астрономическим словом — это намеренно.
Контекст устраняет двусмысленность: «аркан 19 (Солнце)» — про карту,
«солнечный свет» — про светило. Обе формы валидны.
При нумерации N всегда берёшь {{N}} → имя из таблицы выше.
...
def _prompt_karmic_tail(ctx: V3Context) -> tuple[str, int]:
    ...
    return (
        ...
        "именно для читателя на УРОВНЕ ЕГО ОСЕЙ, какие сценарии и "
        "ловушки. Канон даёт общие проявления — твой текст должен "
        "называть СПЕЦИФИЧНЫЕ ситуации, привязанные к тройке арканов.\n"
        "3) ПРАКТИКИ — 3-4 шага СВОИМИ словами, привязанные к "
