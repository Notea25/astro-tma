...
- Числа — символическая рамка, не доказательство фактов биографии.
- Не выдумывай события семьи, детства, зависимости, диагнозы и прошлые жизни.
- Формулируй спорные темы как возможность или вопрос для самонаблюдения.
...
def _build_user_prompt(
    ...
    parts = [
        ...
        _channel_line("Геометрические точки нижнего луча (не программа)", ch["karmic_tail"]),
        (
            "- Каноническая кармическая программа: "
            f"{positions['karmic_program']['key']} — "
            + " → ".join(_name(n) for n in positions["karmic_program"]["arcana"])
        ),
...
async def generate_interpretation(
    ...
    try:
        ...
        fact_context = MatrixFactContext.from_positions(positions)

        async def _call(prompt: str) -> dict[str, Any] | None:
            ...
            for block in message.content:
                if (
                    ...
                    and getattr(block, "name", "") == "publish_reading"
                ):
                    value = getattr(block, "input", None)
                    return value if isinstance(value, dict) else None
            return None

        tool_input = await _call(user_prompt)
        ...
        errors = validate_matrix_text(" ".join(sections.values()), fact_context)
        if errors:
            retry = await _call(
                user_prompt + "\n\nИсправь отклонённый ответ:\n- " + "\n- ".join(errors)
            )
            retry_sections = {
                key: str((retry or {}).get(key, "")).strip() for key in SECTION_KEYS
            }
            retry_errors = validate_matrix_text(
                " ".join(retry_sections.values()), fact_context
            )
            if retry is None or retry_errors:
                sections = {
                    key: safe_symbolic_fallback(SECTION_LABELS_RU[key])
                    for key in SECTION_KEYS
                }
            else:
