...
def _aspect_quality_fallback(p1: str, p2: str, atype: str, orb: float) -> str:
    ...
    orb_line = (
        f"Орб {orb:.1f}° делает аспект заметным и личным: он проявляется не только в редких кризисах, "
        "а в повторяющихся решениях, интонациях и способе отвечать на давление."
        if orb <= _FULL_ASPECT_ORB
        else f"Орб {orb:.1f}° шире, поэтому аспект включается волнами: сильнее всего он виден в ситуациях выбора, отношений и долгих проектов."
    )
    return (
