...
async def _generate_full(title: str, short: str, category: str) -> str:
    ...
    try:
        ...
        prompt = (
            ...
            f"Термин: «{title}» (категория: {category}).\n"
            f"Краткое определение для контекста: {short}\n\n"
            f"Напиши развёрнутую статью на 120-180 слов. Расскажи, что это такое "
            f"простыми словами, как это проявляется в жизни и как этим пользоваться "
            f"(или на что обратить внимание). 2-3 коротких абзаца.\n\n"
            f"КАК ПИСАТЬ:\n"
            f"- Живой язык, как будто объясняешь другу.\n"
            f"- Конкретные примеры из жизни.\n"
            f"- Без жаргона: ни «энергии», ни «вселенная», ни «архетипы».\n"
