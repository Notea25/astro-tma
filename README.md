...
Натальный LLM-текст и уже отрендеренный PDF сохраняются в PostgreSQL. Повторное
скачивание использует готовые байты без нового LLM-запроса и без запуска
Chromium. Кеш автоматически инвалидируется по SHA-256 при изменении данных
рождения, пола, карты/колеса, версии контента или версии PDF-шаблона.
